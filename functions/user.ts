...
async function getUserById(ev: Event) {
  ...
  try {
    const user = await User.findById(new mongoose.Types.ObjectId(id)).select('-password -__v')
...
async function deleteUser(ev: Event) {
  const id = ev.queryStringParameters?.id
  if (id === undefined) return errorRes(400, 'id is required')
  if (!mongoose.isValidObjectId(id)) return errorRes(400, 'invalid id')

  try {
    const { deletedCount } = await User.deleteOne({ _id: new mongoose.Types.ObjectId(id) })
